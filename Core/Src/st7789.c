...
void ST7789_SetRotation(uint8_t m)
{
    ...
    case 0: // Portrait
        ...
        break;
    case 1: // Landscape (THIS IS WHAT YOUR DSO USES)
        // 0x70 = MY | MV | RGB
...
static void ST7789_SetAddressWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
	...
	uint8_t x_data[] = {x_start >> 8, x_start & 0xFF, x_end >> 8, x_end & 0xFF};
	ST7789_WriteData(x_data, sizeof(x_data));

	// Row Address set (Y-axis)
	ST7789_WriteCommand(ST7789_RASET);
	uint8_t y_data[] = {y_start >> 8, y_start & 0xFF, y_end >> 8, y_end & 0xFF};
	ST7789_WriteData(y_data, sizeof(y_data));

	// Write to RAM (Prepare to receive colors)
	ST7789_WriteCommand(ST7789_RAMWR);

	// DO NOT UNSELECT HERE.
	// The ST7789 needs the CS pin to stay LOW while the pixels are being sent.
}
